...
type api_method = "GET" | "POST" | "PUT" | "PATCH" | "DELETE";

interface api_request_options {
  method?: api_method;
  body?: unknown;
...
export async function api_request<T = unknown>(
  ...
  if (!response.ok) {
    throw new api_error(
      response.status,
      get_api_error_message(payload, response.status),
      payload,
    );
  }
  return payload as T;
...
async function read_api_payload(response: Response): Promise<unknown> {
...
function get_api_error_message(payload: unknown, status: number): string {
